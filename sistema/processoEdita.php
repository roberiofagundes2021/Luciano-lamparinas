...
$_SESSION['PaginaAtual'] = 'Editar Processo';
...
$sqlUser = "SELECT ProceId,ProceNumero,ProceDataAutuacao,ProceEspecie,ProceUrgente,ProceTipoProcesso,
    ...
    FROM Processo
	WHERE ProceEmpresa = $iEmpresa AND ProceId = $iProcesso";
$processos = $conn->query($sqlUser);
$processo = $processos->fetch(PDO::FETCH_ASSOC);

$sql = "SELECT UnidaId,UnidaNome
	FROM Unidade
	WHERE UnidaId = $processo[ProceUnidadeGestora]";
...
<head>
	...
	<title>Lamparinas | Editar Processo</title>
		...
		$(document).ready(function(){
			$('#formProcesso').submit(function(e){
				...
				$.ajax({
					...
					dataType: 'json',
					data: {
						'tipoRequest':'ATTPROCESSO',
						'iProcesso': processo.ProceId,
						'numero': $('#numero').val(),
						'data': $('#data').val(),
						'especie': $('#especie').val(),
						'urgente': $('#urgente').val(),
		...
		function getCmb(processo){
			...
			$.ajax({
				...
				success: function(response) {
					...
					$('#tipo').val(processo.ProceTipoProcesso).change()
					$('#categoria').val(processo.ProceCategoria).change()
					$('#status').val(processo.ProceStatus).change()
					$('#situacao').val(processo.ProceSituacao).change()

					$('#numero').val(processo.ProceNumero)
					$('#data').val(processo.ProceDataAutuacao)
					$('#especie').val(processo.ProceEspecie).change()
					$('#urgente').val(processo.ProceUrgente?'S':'N').change()
					$('#descricao').val(processo.ProceDescricao)
					$('#responsavel').val(processo.ProceResponsavelNome)
					$('#telefone').val(processo.ProceResponsavelTelefone)
					$('#email').val(processo.ProceResponsavelEmail)
				}
			})
		}
	</script>
